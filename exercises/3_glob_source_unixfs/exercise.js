...
exercise.addVerifyProcessor(verifyProcessor(exercise, async (test) => {
  ...
  const cid = await solutionFn(path.join(__dirname, 'test-data'), '**/*')

  await test.equals(cid.toString(), 'bafybeiawmlbbooqojiyyco4lckc2ujo26fqar6zqajoxtzsqccnjbvsm5y', 'correct_cid')
}))
